...
In each environment, two EC2 instances are created and they are supported by a VPC, a database and an application load balancer.
...
It is also worth noting that https is not yet enabled (the EC2 instances are connected by http for now)
...
There is now a new file "environ/$ENV/cost_with_DBS.txt" (for example -- environ/qa/cost_with_DBS.txt) that shows the estimated monthly cost of running the infrastructure system with the database when it is setup.
...
Also, please note that the security groups of the EC2 instances have been updated to be more secure
As a result, the EC2 instances can not be reached directly in the browser and will have to be contacted (ec2 can only be reached) via the DNS of the loadbalancer on the browser

We can still SSH into the EC2 instances though

There is no cloudfront, not cognito nor dynamoDB for now
There is also no auto scaling
