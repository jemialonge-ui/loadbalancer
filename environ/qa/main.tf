...
module "qa_security_group" {
  ...
  rds_security_group_id = module.qa_db_security_group.security_group_id
}
...
module "qa_database" { ##RDS database needs to be in a private subnet, so using the first private subnet for the database subnet group  
  source              = "../../database"
  ...
  db_security_group_ids = [module.qa_db_security_group.security_group_id]
  db_subnet_ids       = module.vpc_qa.Private_subnet_ids
...
module "ec2_qa1" {
  ...
  {
    EC2_server_name = "EC2_QA1"
    DB_ENDPOINT = module.qa_database.db_endpoint
